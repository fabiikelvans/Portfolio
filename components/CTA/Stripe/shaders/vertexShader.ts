...
void main() {
    ...
    float noise = snoise(vec3(noiseCoord.x + u_time * 0.15, noiseCoord.y, u_time * 0.25));
    ...
    for(int i = 0; i < 4; i++) {
        ...
        float noiseFlow = 0.15 + float(i)*0.13 ;
        float noiseSpeed= 0.25 + float(i)*0.13;

        float noiseSeed= 0.15 + float(i)*0.25;
        vec2 noiseFreq = vec2(0.6, 1.2);

        float noiseFloor = 0.1;
        float noiseCeil = 0.6 + float(i) * 0.02;
